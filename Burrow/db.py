...
class BurrowDB:
    ...
    def __init__(self, db_path: str = "./data/burrow.db"):
        ...
        if query is None and type is None:
            ...
            rows1 = self.conn.execute(
                f"""SELECT * FROM entries WHERE {where}
                   AND (event_time>=? OR event_time IS NULL)
                   AND event_time<?
                   ORDER BY importance DESC, event_time DESC
                   LIMIT ?""",
                params + [today_start, (now + timedelta(days=1)).strftime("%Y-%m-%dT00:00:00"), 5]
            ).fetchall()

            # 第2层：昨天 importance >= 6 的记忆
            rows2 = self.conn.execute(
                f"""SELECT * FROM entries WHERE {where}
                   ...
                   ORDER BY importance DESC, event_time DESC
                   LIMIT ?""",
                params + [yesterday_start, today_start, 3]
            ).fetchall()

            # 第3层：近7天 importance >= 7 的记忆
            rows3 = self.conn.execute(
                f"""SELECT * FROM entries WHERE {where}
                   ...
                   ORDER BY importance DESC, event_time DESC
                   LIMIT ?""",
                params + [week_ago, yesterday_start, 2]
            ).fetchall()

            # 第4层：浮动类型（待办等），不限时间
            float_types = self.conn.execute(
                "SELECT type FROM type_config WHERE floats_in_default=1"
            ).fetchall()
            if float_types:
                placeholders = ",".join("?" for _ in float_types)
                ft_conditions = conditions + [f"type IN ({placeholders})"]
                ft_where = " AND ".join(ft_conditions)
                rows4 = self.conn.execute(
                    f"""SELECT * FROM entries WHERE {ft_where}
                       ORDER BY importance DESC, event_time DESC
                       LIMIT ?""",
                    params + [t[0] for t in float_types] + [3]
                ).fetchall()
            else:
                rows4 = []
