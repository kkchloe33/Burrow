...
def _format_entry(entry: dict) -> str:
    ...
    # domain 信息（如果有且不是默认空值）
    domain = entry.get("domain", "")
    domain_str = f" [{domain}]" if domain else ""

    return f"[{event_time}] [{etype}]{domain_str} {title}{fields_str} | ID:{eid}"
...
async def remember(
    ...
    Args:
        content: 记忆正文内容
        type: 记忆类型，可选 general/journal/thought/todo/outfit/diet/period，默认 general
...
async def recall(
    ...
    # 如果有 query 且 embedding 开启，做语义搜索增强
    if query and embedder.enabled:
        query_vec = await embedder.generate_embedding(query)
        if query_vec:
            sem_results = embedder.search_similar(query_vec, entries, top_k=10)
            # 合并：语义结果优先，剩余的加权补充
            sem_ids = {e["id"] for e in sem_results}
            merged = sem_results.copy()
            for e in entries:
