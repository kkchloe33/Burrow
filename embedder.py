...
class Embedder:
    ...
    def search_similar(
        ...
        """
        在传入的 entries 列表中按余弦相似度排序（用于 FTS5 结果内重排序）
    ...
    def search_similar_global(
        ...
        """
        **全局向量搜索**：从数据库中加载所有有 embedding 的条目，按余弦相似度排序。
        区别于 search_similar（仅在传入的 entries 列表中搜索）。

        Args:
            query_vec: 查询向量
            db: BurrowDB 实例，用于加载所有带向量的条目
            top_k: 返回前 N 条
            min_score: 最小相似度阈值

        Returns:
            带 _sem_score 字段的结果列表
        """
        all_entries = db.get_all_with_embedding()
        results = []
