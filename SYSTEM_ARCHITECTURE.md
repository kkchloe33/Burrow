...
recall(query)
  → query 非空 + embedding 启用
    ...
    → embedder.search_similar_global(query_vec, db, top_k=20)
      → 从数据库加载所有有 embedding 的条目，计算余弦相似度
      → 返回相似度 >= 0.3 的结果
```

**技术实现**：
- 向量以二进制 `struct.pack("<Nf", ...)` 紧凑存储
- **全局语义搜索**：FTS5 关键词搜索 + 全局向量搜索（非仅 FTS5 结果内重排序）
- 两组结果按 id 合并去重，再按时间+重要度+语义分加权排序
...
场景 B：有 query（如 recall(query="面试")）
  ...
  3. 全局语义搜索（如果 embedding 启用）：
     - 对 query 生成 query_vec
     - 从数据库加载所有有 embedding 的条目，计算余弦相似度
  4. 合并去重：FTS5 结果 + 向量搜索结果
  5. _calculate_score + _sem_score 加权排序
     total = _calculate_score(e) + _sem_score * 5.0
  6. 触发经期预测（如果 query 含"经期/period"等关键词）
...
5. ~~向量搜索范围小：语义搜索仅在前一步 FTS5 的结果集内重排序，不是全局向量搜索~~ **已修复**：现为 FTS5 + 全局向量搜索合并排序
